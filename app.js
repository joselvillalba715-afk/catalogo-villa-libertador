...
function calcularDescuento() {
  ...
  if (base <= 0) return 0;
  if (cuponAplicado.tipo === "porcentaje") return Math.round(((base * cuponAplicado.valor) / 100) * 100) / 100;
  return Math.min(cuponAplicado.valor, base);
}
...
function textoDescripcionCupon(cupon) {
  ...
  return `Cupón ${cupon.codigo}: ${valorTexto} de descuento${alcanceTexto}`;
}
